# Hub for Millix network

This is a node for Millix network that serves as [relay](../../../millix-relay), plus facilitates exchange of end-to-end encrypted messages among devices connected to Millix network.  The hub does not hold any private keys and cannot send payments itself.
...
* Plain text chat with bots that offer a service and can receive or send payments.  [Faucet](../../../millix-faucet) is an example of such bot.
...
Users set their hub address in their wallet settings.  The default hub is wss://millix.org/bb but users can change it to your hub address.
...
npm install
```
Set up a proxy, such as nginx, to forward all websocket connections on a specific path to your daemon running the hub code.  See example configuration for nginx in [millixcore](../../../millixcore) documentation.
